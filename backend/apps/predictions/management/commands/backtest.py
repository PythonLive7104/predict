    ...
    def _report(self, tally, scored, warmup, min_conf):
        ...
        if priced < 30:
            self.stdout.write(self.style.WARNING(
                ...
                "  Strike rate alone decides nothing: a market can win three times\n"
                "  in four and still lose money at short prices.\n"
        ...
        # The overall figure is a weighted average across markets, and double
        # chance — two outcomes out of three — drags it upward whenever it
        # dominates the mix. Checking the hardest market instead is what
        # actually distinguishes a real result from graded-against-itself.
        one_x_two = tally.get(Market.MATCH_RESULT, {"n": 0, "won": 0})
        hardest = (one_x_two["won"] / one_x_two["n"] * 100) if one_x_two["n"] else 0

        if hardest >= 65:
            self.stdout.write("")
            self.stdout.write(self.style.WARNING(
                f"  ⚠  {hardest:.1f}% on match results is implausibly high for football.\n"
                "     Best-in-class tops out near 60%. If these fixtures were seeded\n"
                "     rather than pulled from API-Football, the model is being graded\n"
                "     against data generated by its own assumptions and this measures\n"
                "     nothing."
            ))
        elif overall >= 65:
            dc = tally.get(Market.DOUBLE_CHANCE, {"n": 0})
            share = dc["n"] / total_n * 100 if total_n else 0
            self.stdout.write("")
            self.stdout.write(
                f"  {overall:.1f}% overall is flattered by the market mix: double chance\n"
                f"  is {share:.0f}% of these picks and combines two outcomes out of three,\n"
                f"  so it clears 70% by construction. Match results — the hard market —\n"
                f"  came in at {hardest:.1f}%, which is the number worth quoting."
            )
